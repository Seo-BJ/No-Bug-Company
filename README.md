## GitHub 가이드라인

관련 자료 링크 : 

### 브랜치 정책

1. master
	- 설명 : 배포 가능한 빌드버전에 대한 브랜치, Develop 외에 다른 branch에서는 Merge 되지 않는다.
	- 보호 정책 : 기본적으로 Lock 상태, 1~2주 간격으로 Build를 위해 Devlop에서 최종 작업물들을 Merge한다.

2. develop
	- 설명 : 개발이 현재 진행되는 브랜치, feature에서 작업이 완료되어 Merge된 작업물들을 포함한다.
	- 보호 정책 : 기본적으로 Lock 상태, Featire에서 작업물들을 Pull Request와 코드 리뷰(미정)을 받아서 Approve되면, 이를 Merge 한다.
(아래 작성 중)
3. feature
	- 설명 : 이슈로부터 생성되어, develop 브랜치를 Base로 하여 각 개발자의 Local에서 작업을 수행하는 Branch이다. 코드 리뷰(미정) 후 develop 브랜치로 Pull Requeset가 승인되면, develop에 merge된다. (그 후 이슈와 branch는 삭제)
	- base 브랜치/상위 브랜치 : develop 브랜치
	- 보호 정책 : 각 개발자가 소유하는 Local 브랜치 개념으로, 서로의 브랜치는 최대한 침범하지 않는다. 
	- branch 이름 : <이슈 Label 종류>-<이슈이름><이슈번호>
	- 예시 : feature-CharacterLocomotion#5, fix-CharacterLocomotion#5

************************************************************************************************************************************************************************

### Issue와 깃허브 사용 정책
4. Issue
  	- 이슈 종류 :
		1. Feat : 기능 추가, 구현
		2. Fix : 버그 수정 
		3. Asset : 애셋 추가/수정/변경 (비 개발자 전용)
		4. Refactoring : 리팩토링 작업
		5. Etc : 그 외
	- 이슈 제목 : <이슈 Label 종류> : <제목>
	- 예시 : Feat : 플레이어 케릭터 애니메이션 추가
	- 이슈 본문 : 템플릿 활용
5. Task 할당부터 Develop Merge까지 과정
	 - 순서
  		1. 회의를 통해 결정된 Task 할당, 그에 대한 Issue 발행
		2. GitHub Project에서 해당 이슈를 In-Progress 열로 옮긴다.
		3. Assignee 설정 - 작업하는 본인으로 변경
		3. Create Branch 
		4. Base 브랜치를 Develop으로 설정 
		5. Local에서 작업
		6. 작업 완료 후 Commit 작성 및 Push (원격 저장소의 동일 네임의 feature 브랜치로 push)
		7. develop < - feature PR 작성 후, 리뷰 받기
		8. 모든 리뷰어로부터 Approval 받으면, Owner가 Merge 진행

************************************************************************************************************************************************************************

2. Commit Convention
	1. Commit 제목 : branch 이름과 동일하게
	2. Commit 본문 : 간략하게 작성.

************************************************************************************************************************************************************************

3. PR 작성
	1. 제목 작성 
		- <이슈 Label 종류> : <제목>
	2. 본문
		- 간략하게 작성. 알려야 하는 내용이 있으면 반드시 포함.
************************************************************************************************************************************************************************
