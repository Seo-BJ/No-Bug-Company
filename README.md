...
No Bug Company 프로젝트 리포지토리 입니다. 소스 코드 구조 및 클래스 역할에 대한 개요입니다.

## 프로젝트 개요

PestControl은 언리얼 엔진으로 제작된  2개의 스테이지로 이루어진 탑다운 생존 로그라이크 게임입니다. 
...
### 콘텐츠 (`Content`)
언리얼 엔진 에셋은 다음과 같이 구성됩니다:
- `Content/Blueprints`: 블루프린트로 구현된 모든 게임 플레이 로직을 포함합니다. 이는 C++ 폴더와 같은 구조를 가집니다.
- `Content/Assets`: 게임에 사용되는 모든 아트, 사운드 및 시각 효과 에셋을 포함합니다.
- `Content/Maps`: 게임의 레벨(또는 맵)을 포함합니다.
- `Content/Data`: `DataTable` 에셋에 사용되는 `.csv` 파일을 포함합니다. 이곳에 게임의 밸런스 데이터(플레이어, 적, 무기 등의 스탯)가 저장됩니다.
